...
function renderTest(test) {
  ...
  test.problems.forEach((p, i) => {
    ...
    div.appendChild(pEl);
    if (test.format === 'multiple-choice' && Array.isArray(p.choices)) {
...
function gradeTest(test, form) {
  ...
  const container = document.getElementById('quiz');
  container.innerHTML = `<p>You scored ${correct} out of ${test.problems.length}.</p>`;
  ...
  const summary = { correct, total: test.problems.length, strengths, weaknesses };
  try {
